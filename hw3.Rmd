...
```{r}
ggplot(tips, aes(x=tips$total_bill, y=tips$tip)) + geom_point() + labs(title="Total Bill vs Tip", x = "Total Bill", y = "Tip")
```
...
ggplot(tips, aes(x=tips$time, y=tips$tiprate)) + geom_boxplot() + labs(x="Time", y="tiprate", title="Time vs Tip Rate")
```
...
ggplot(tips, aes(x=tips$ss, y = tips$tiprate)) + geom_boxplot() + labs(x="Smoker/Sex", y="Tip Rate", title = "Smoker/Sex vs Tip Rate")
```
...
```{r}
ggplot(tips, aes(fill=tips$sex, y=1,x=tips$day)) + geom_bar(position="fill", stat="identity") + labs(title="Sex of Patron vs Day of the Week",y="Percentage", x="Day of the Week", fill="Sex")
```
